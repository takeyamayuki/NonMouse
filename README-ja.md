...
# 特徴
- 全く新しいマウス  
   ソフトウェアからのアプローチのマウスです。手を認識してマウスを動かします。
- トラックパッドやマウスよりは、絵が描きやすい  
   macのトラックパッドは力を下方向に入れっぱなしなので、描きにくい。
- タイピングとの相性が良い  
   アプリケーションが非アクティブな状態でも、グローバルホットキーによりNonMouseを呼び出すことができます

- 場所を選ばず、姿勢が自由
- タッチディスプレイ化することもできる
   webカメラをディスプレイに向けることで、タッチディスプレイっぽいものができる

- 最新のリリースからダウンロードするだけで使える  
   (windows, macのみ)
# インストール
## 📁 実行可能形式ファイルとして実行
