...
class Frame(wx.Frame):
    ...
    def __init__(self):
        ...
        # ここで Alt/option+Space のホットキーを登録しています。
        new_id = wx.NewIdRef(count=1)
        self.RegisterHotKey(new_id, wx.MOD_ALT, wx.WXK_SPACE)
        self.Bind(wx.EVT_HOTKEY, self.Foo, id=new_id)

        # ここで Shift+Space のホットキーを登録しています。
        new_id = wx.NewIdRef(count=1)
        self.RegisterHotKey(new_id, wx.MOD_SHIFT, wx.WXK_SPACE)
        self.Bind(wx.EVT_HOTKEY, self.Bar, id=new_id)

    def Foo(self, event):
        global i_break
        dis = 0.7               # くっつける距離の定義
        preX = preY = 0         # 前回のx,y座標
        nowCli = preCli = 0     # 現在、前回の左クリック状態
        norCli = prrCli = 0     # 現在、前回の右クリック状態
        douCli = 0              # ダブルクリック状態
        i = k = 0
        LiTx = []
        LiTy = []
        ...
        # メインループ
        while cap.isOpened() and i_break == 0:
